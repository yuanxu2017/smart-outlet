...
class Client extends EventEmitter {
  ...
  constructor (options) {
    ...
    this.socket.on('message', function (msg, rinfo) {
      ...
      if (this.devices.has(sysinfo.deviceId)) {
        ...
        plug.status = 'online';
        plug.host = sysinfo.host;
        plug.seenOnDiscovery = this.discoveryPacketSequence;
        this.emit('plug-online', plug);
      } else {
        const plug = new Plug({client: this, deviceId: sysinfo.deviceId, host: rinfo.address, port: rinfo.port, seenOnDiscovery: this.discoveryPacketSequence,timeout:this.timeout});
  ...
  sendDiscovery (devices) {
   // console.error(this.discoveryInterval);
    devices = devices || [];
